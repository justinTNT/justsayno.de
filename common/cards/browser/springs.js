...
var $vor;
	...
	$.validator.addMethod('ntnumber', function(val, el){
			return this.optional(el) || /^(08(\-)?)?8[0-9]{3}([ \-])?[0-9]{4}$/.test(val);
		}, "Please enter a valid NT number, in the form 08-8xxx-xxxx");
		...
		if ($vor.form())
			...
			justsayAJAJ('/verify/' + user.handle,
						function(success){
							...
							alert("You need to confirm your alicesprings.email account first.");
						});
		});
